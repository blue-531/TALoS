...
def get_pc_model_class(name):
    global REGISTERED_PC_DATASET_CLASSES
    # print(REGISTERED_PC_DATASET_CLASSES)
...
class SemKITTI_sk_multiscan(data.Dataset):
    def __init__(self, data_path, imageset='train',stride=5,return_ref=False, label_mapping="semantic-kitti-multiscan.yaml", nusc=None,sq_num=10):
        self.return_ref = return_ref
        with open(label_mapping, 'r') as stream:
            semkittiyaml = yaml.safe_load(stream)
        ...
        self.times = []
        self.poses = []
        self.use_test_time_adaptation=True
    ...
    def frame_transform_scan(self, points, pose0, pose):
    ...
    def __getitem__(self, index):
        ...
        prev_annotated_data=[]
        stride_list=[]
        for stride_ in self.stride:
            data_idx = number_idx + stride_ 
            
            if data_idx<0:  
                continue
            
            path = self.im_idx[index][:-10]
            newpath_prev = path + str(data_idx).zfill(6) + self.im_idx[index][-4:]
            voxel_path = path.replace('velodyne', 'labels')
            files=list(pathlib.Path(path).glob('*.bin'))
            
            if data_idx < len(files):
                pose = self.poses[dir_idx][data_idx]
                pose_list.append(pose)
                raw_data_prev = np.fromfile(newpath_prev, dtype=np.float32).reshape((-1, 4))
                prev_scans.append(raw_data_prev)
                annotated_data_prev = np.zeros([256, 256, 32], dtype=int).reshape((-1, 1))
                annotated_data_prev = self.comletion_remap_lut[annotated_data_prev]
                annotated_data_prev = annotated_data_prev.reshape((256, 256, 32))
                prev_annotated_data.append(annotated_data_prev)
                stride_list.append(stride_)
                
        if len(pose_list)>1:
            prev_annotated_data=np.stack(prev_annotated_data,0)
            prev_scans_xyz=[prev_scan[:, :3] for prev_scan in prev_scans]
            prev_scans_i=[prev_scan[:, 3] for prev_scan in prev_scans]
            prev_data_tuple=(prev_scans_xyz, prev_annotated_data.astype(np.uint8))
            prev_data_tuple += (prev_scans_i,origin_len)
        else:
            prev_data_tuple=(np.zeros([1,1,1]),np.zeros([1,1,1]),np.zeros([1,1,1]),1)
        ...
        return data_tuple,prev_data_tuple,pose_list,stride_list
