...
def no_extraction(x):
    device = x.device
    total_index=torch.ones_like(torch.sum(torch.abs(x), dim=1)).nonzero()
    coords = total_index.type(torch.int32).to(device)
    channels = int(x.shape[1])
    features = x.permute(0, 2, 3, 4, 1).reshape(-1, channels)
    total_index2=torch.ones_like(torch.sum(torch.abs(features), dim=1)).nonzero()
    features = features[total_index2, :]
    features = features.squeeze(1).to(device)
    coords, _, _ = torch.unique(coords, return_inverse=True, return_counts=True, dim=0)
    return coords, features

# def manual_extraction(x, idx_for_extract):
#     device = x.device
#     nonzero_index = torch.sum(torch.abs(x), dim=1).nonzero()
#     manually_added_index = (idx_for_extract==1).nonzero()
#     total_index = torch.cat((nonzero_index, manually_added_index), dim=0)
#     total_index, _, _ = torch.unique(total_index, return_inverse=True, return_counts=True, dim=0)
#     coords = total_index.type(torch.int32)
    
#     channels = int(x.shape[1])
#     features = x.permute(0, 2, 3, 4, 1).reshape(-1, channels)
#     nonzero_index2 = torch.sum(torch.abs(features), dim=1).nonzero()
#     manually_added_index2 = (idx_for_extract.flatten()==1).nonzero()
#     total_index2 = torch.cat((nonzero_index2, manually_added_index2), dim=0)
#     total_index2, _, _ = torch.unique(total_index2, return_inverse=True, return_counts=True, dim=0)
#     features = features[total_index2, :]
#     features = features.squeeze(1).to(device)
#     # coords, _, _ = torch.unique(coords, return_inverse=True, return_counts=True, dim=0)
#     return coords, features

def manual_extraction(x, proposure):
    device = x.device
    #proposure.shape -> (n,3)
    x_proposure=x.clone().detach()
    # import pdb;pdb.set_trace()
    if proposure!=None:
        x_proposure[0,0,proposure[:,0],proposure[:,1],proposure[:,2]]=1
        total_index=torch.sum(torch.abs(x_proposure), dim=1).nonzero()
    ...
    channels = int(x.shape[1])
    features = x.permute(0, 2, 3, 4, 1).reshape(-1, channels)
    features_proposure = x_proposure.permute(0, 2, 3, 4, 1).reshape(-1, channels)
    if proposure==None:
        total_index=torch.ones_like(torch.sum(torch.abs(features), dim=1)).nonzero()
    else:
        total_index=torch.sum(torch.abs(features_proposure), dim=1).nonzero()
    features = features[total_index, :]
    features = features.squeeze(1).to(device)
    coords, _, _ = torch.unique(coords, return_inverse=True, return_counts=True, dim=0)
    return coords, features
...
class Asymm_3d_spconv(nn.Module):
    ...
    def forward(self, voxel_features, coors, batch_size, extraction='all'):
        ...
        # Dense to sparse
        if extraction is 'all':
            coord, features = extract_nonzero_features(x)
        elif extraction is None:
            coord, features = no_extraction(x)
        else:
            coord, features = manual_extraction(x, extraction)
